...
class Controller:
    def __init__(
        ...
        self.kIw = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32, device=gravity.device)
        self.kDw = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32, device=gravity.device)

        self.w_error_integral_max = torch.tensor([0.1, 0.1, 0.1], dtype=torch.float32, device=gravity.device)
        self.w_error_integral = torch.zeros(num_envs, 3, dtype=torch.float32, device=gravity.device)
        self.w_error_prev = torch.zeros(num_envs, 3, dtype=torch.float32, device=gravity.device)

        self.K_min_norm_collec_acc = 3.0
        self.K_max_ang = k_max_ang
        self.K_max_bodyrates_feedback = 13.0
        self.K_max_angular_acc = 200.0
        ...
        self.thrust_old = torch.full((num_envs,), float((self.mass * self.gravity.norm()).item()), dtype=torch.float32, device=self.gravity.device)
    ...
    def reset(self, env_ids: torch.Tensor | None = None):
        if env_ids is None:
            self.w_old.zero_()
            self.thrust_old.fill_(float((self.mass * self.gravity.norm()).item()))
            self.w_last.zero_()
            self.w_error_integral.zero_()
            self.w_error_prev.zero_()
            return

        self.w_old[env_ids] = 0.0
        self.thrust_old[env_ids] = float((self.mass * self.gravity.norm()).item())
        self.w_last[env_ids] = 0.0
        self.w_error_integral[env_ids] = 0.0
        self.w_error_prev[env_ids] = 0.0
    ...
    def get_control(
        ...
        pid_error_accelerations = compute_pid_error_acc(
            p_odom, v_odom, p_desired, v_desired, self.kPp, self.kPv
        )
        ...
        thrust_force = quat_apply(
            q_desired,
            thrust_desired.unsqueeze(1) * torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device=thrust_desired.device),
        )
        total_des_acc = compute_limited_total_acc_from_thrust_force(
            thrust_force,
            self.mass,
            self.K_min_norm_collec_acc,
            self.K_max_ang,
        ...
        feedback_bodyrates = compute_feedback_control_bodyrates(
            q_odom, q_desired, self.kPR, self.K_max_bodyrates_feedback
        )

        if env_ids is None:
            w_desired = compute_limited_angular_acc(
                w_desired + feedback_bodyrates,
                self.w_last,
                self.K_max_angular_acc,
                self.step_dt,
            )
            self.w_last = w_desired
            thrust_desired, torque_desired = self.bodyrate_control(
                q_odom, w_odom, force_desired, w_desired
            )
        else:
            w_desired = compute_limited_angular_acc(
                w_desired + feedback_bodyrates,
            ...
            thrust_desired, torque_desired = self.bodyrate_control(
                q_odom, w_odom, force_desired, w_desired, env_ids
            )
    ...
    def minimum_singularity_flat_with_drag(
        ...
        if not bool(torch.all(success)):
            failed_count = int((~success).sum().item())
            LOGGER.warning("Flatness controller fell back to previous command for %d environments", failed_count)
    ...
    def bodyrate_control(
        ...
        rotation = matrix_from_quat(q_odom)
        thrust_desired = (force_desired * rotation[:, :, 2]).sum(dim=1)

        inertia = self.inertia.view(3, 3)
        inertia_w_odom = torch.matmul(inertia, w_odom.transpose(0, 1))
        feedforward = torch.stack(
            [
                w_odom[:, 1] * inertia_w_odom[2, :] - w_odom[:, 2] * inertia_w_odom[1, :],
                w_odom[:, 2] * inertia_w_odom[0, :] - w_odom[:, 0] * inertia_w_odom[2, :],
                w_odom[:, 0] * inertia_w_odom[1, :] - w_odom[:, 1] * inertia_w_odom[0, :],
            ],
            dim=1,
        )
        ...
        if env_ids is None:
            ...
            self.w_error_integral = torch.max(
                torch.min(self.w_error_integral, self.w_error_integral_max),
                -self.w_error_integral_max,
            )
            ...
            torque_desired = (
                feedforward
                + self.kPw * w_err
                + self.kIw * self.w_error_integral
                + self.kDw * w_err_deriv
            )
        else:
            self.w_error_integral[env_ids] += w_err * self.step_dt
            self.w_error_integral[env_ids] = torch.max(
                torch.min(self.w_error_integral[env_ids], self.w_error_integral_max),
                -self.w_error_integral_max,
            )
            ...
            torque_desired = (
                feedforward
                + self.kPw * w_err
                + self.kIw * self.w_error_integral[env_ids]
                + self.kDw * w_err_deriv
            )
...
def compute_pid_error_acc(
    p_odom: torch.Tensor,
    v_odom: torch.Tensor,
    p_desired: torch.Tensor,
    v_desired: torch.Tensor,
    kPp: torch.Tensor,
    kPv: torch.Tensor,
) -> torch.Tensor:
    pos_error = torch.where(
        torch.isnan(p_desired),
        torch.zeros_like(p_desired, dtype=torch.float32),
        p_desired - p_odom,
    )
...
def compute_limited_total_acc_from_thrust_force(
    ...
    mass: torch.Tensor,
    k_min_norm_collec_acc: float,
    k_max_ang: float,
) -> torch.Tensor:
    total_acc = thrust_force / mass
    ...
    if k_max_ang > 0:
        z_acc = total_acc[:, 2]
        z_acc = torch.where(z_acc < k_min_norm_collec_acc, torch.full_like(z_acc, k_min_norm_collec_acc), z_acc)

        z_body = total_acc / (total_acc.norm(p=2, dim=1, keepdim=True) + 1e-6)
        unit_z = torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device=total_acc.device).expand(z_body.shape[0], 3)
        rot_axis = torch.cross(unit_z, z_body, dim=1)
        rot_axis = rot_axis / (rot_axis.norm(p=2, dim=1, keepdim=True) + 1e-6)

        rot_ang = torch.acos(z_body[:, 2].clamp(-1.0, 1.0))
        k_max_ang_rad = math.radians(k_max_ang)
        mask = rot_ang > k_max_ang_rad
        if mask.any():
            limited_z_body = unit_z * math.cos(k_max_ang_rad) + torch.cross(rot_axis, unit_z, dim=1) * math.sin(k_max_ang_rad)
            new_total_acc = (z_acc / math.cos(k_max_ang_rad)).unsqueeze(1) * limited_z_body
...
@torch.jit.script
def compute_feedback_control_bodyrates(
    q_odom: torch.Tensor,
    q_desired: torch.Tensor,
    kPR: torch.Tensor,
    k_max_bodyrates_feedback: float,
):
    q_err = quat_mul(quat_inv(q_odom), q_desired)
    axis_angle_err = axis_angle_from_quat(q_err)
    bodyrates = kPR * axis_angle_err
    bodyrates = torch.clamp(
        bodyrates,
        min=-k_max_bodyrates_feedback,
        max=k_max_bodyrates_feedback,
    )
    return bodyrates


@torch.jit.script
def compute_limited_angular_acc(
    candidate_bodyrate: torch.Tensor,
    w_last: torch.Tensor,
    k_max_angular_acc: float,
    step_dt: float,
):
